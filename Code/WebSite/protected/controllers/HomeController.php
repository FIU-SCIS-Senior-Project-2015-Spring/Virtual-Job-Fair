...
//                }
        ...
        /**
         * Used to delete Adv. Search Queries for the employer.
         * Corrects bug card #626.
         */
        public function actionDeleteAdvSearchQueryEmp()
        {
            $model = SavedQuery::model()->findByPk($_GET['id']);
            $model->delete();
            $this->redirect('/JobFair/index.php/job/emphome');
        }

        /**
         * Used to delete Adv. Job Search Queries for Student.
         * Corrects bug card #671.
         */
        public function actionDeleteAdvSearchQueryStu()
        {
            $model = SavedQuery::model()->findByPk($_GET['id']);
            $model->delete();
        ...
        $request = Yii::app()->curl->run('http://www.json-generator.com/api/json/get/bRQiTpYSCq?indent=2');
            ...
            foreach ($job_postings as $job_posting)
            {
                ...
                $dup_entries = Job::model()->find("FK_poster=:poster AND " .
                    ...
                    "post_date=:post_date", array(':poster' => $current_user->id,
                    ':title' => $jp_position,
